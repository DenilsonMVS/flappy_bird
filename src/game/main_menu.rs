...
use crate::{Atlas, AtlasFrame, FontLibrary, MainContext, NextScene, TextureLibrary, game::{button::Button, defs::{BIRD_START_POSITION, HORIZONTAL_SPEED, SCENE_HEIGHT}, pipes::Pipes, scene::Scene}, graphics::renderer::{BlendFactor, Capability, ClearField, Renderer, fonts::FontVbo, positioning::{BaseDimensions, PositionMode, scale_dimension}, simple_texture::SimpleTexture}};
...
impl<'a> Scene for MainMenu<'a> {
    ...
    fn game_logic(&mut self, context: &mut MainContext) {
        self.pipes.game_logic(self.x_offset, &mut self.rng);
